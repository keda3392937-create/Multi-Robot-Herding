"""Live visualization launcher for the dynamic 4+4 V5.2 baseline."""
...
from baseline_two_group_v5_2_standalone import HerdingSimulation as V52Simulation


class VisualV52Simulation(V52Simulation):
    """V5.2 policy with live group colors and regroup highlighting."""
    ...
    def __init__(
        ...
        *,
        ...
        self._lastVisualGroups: tuple[int, ...] | None = None
        self._herderLabels = []
        self._regroupHighlight = None
        super().__init__(
        ...
        self._lastVisualGroups = tuple(int(value) for value in self.herderGroups)

    def setup_visualization(self) -> None:
        super().setup_visualization()
        manager = getattr(self.fig.canvas, "manager", None)
        if manager is not None and hasattr(manager, "set_window_title"):
            manager.set_window_title("V5.2 dynamic 4+4 baseline")
        ...
        self._regroupHighlight = self.ax.scatter(
            [],
            [],
            s=220,
            facecolors="none",
    ...
    def update_visualization(self, current_time: float) -> bool:
        ...
        current_groups = tuple(int(value) for value in self.herderGroups)
        groups_changed = current_groups != self._lastVisualGroups
        if groups_changed:
            self._lastVisualGroups = current_groups
        ...
        events = self.policy.event_history
        last_event_text = "No runtime regroup yet"
        if events:
            event = events[-1]
            before = event["before"]
            after = event["after"]
            ...
            if self._regroupHighlight is not None:
                self._regroupHighlight.set_offsets(
                    self.positionsHerder[np.asarray(changed_indices, dtype=int)]
                )
            names = ", ".join(f"H{index}" for index in changed_indices)
            last_event_text = f"last regroup t={float(event['time']):.1f}s: {names}"
        elif self._regroupHighlight is not None:
            self._regroupHighlight.set_offsets(np.empty((0, 2), dtype=float))
        ...
        self.ax.set_title(
            f"V5.2 dynamic 4+4 | t={display_time:.1f}s | "
            f"in target={count}/{self.numEvaders} | "
            f"regroups={self.policy.reassignment_events}\n"
...
def main() -> None:
    ...
    simulation = VisualV52Simulation(
